...
from plot_libs.graph_lib import get_plot_compare_with_base_line_cfg, get_base_annotations
from query_api.parse_pre_res import FetchGroundTruth
from query_api.query_train_base_api import post_processing_train_base_result
...
if __name__ == "__main__":
    ...
    fgt = FetchGroundTruth(space_name=args.search_space, total_epoch=200)
    ...
    for run_id in run_range_:
        ...
        for Tmin in budget_array:
            best_arch, _, _, _ = \
                RunModelSelection(args, fgt, args.dataset, is_simulate=True).\
                    select_model(Tmin*60, run_id, only_phase1=False)
    ...
    for run_id in run_range_:
        ...
        for Tmin in budget_array_p1:
            ...
            best_arch_p1, _, _, _ = \
                RunModelSelection(args, fgt, args.dataset, is_simulate=True).\
                    select_model(Tmin * 60, run_id, only_phase1=True)
