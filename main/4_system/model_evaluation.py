    ...
    for run_id in range(args.num_run):
        ...
        try:
            while True:
                ...
                new_arch = used_search_space.copy_architecture(int(arch_id), _, new_bn=True)
                ...
                new_arch = used_search_space.copy_architecture(int(arch_id), _, new_bn=False)
                new_arch = new_arch.to(args.device)
                ...
                sampler.add_model_to_rank(arch_id, alg_score)
                sampler.fit_sampler()
                ...
                top_arch_ids = sampler.get_current_top_10()
                y_axis_top10_models.append(top_arch_ids)
                # simulate training process, when i < 10, train all of them
