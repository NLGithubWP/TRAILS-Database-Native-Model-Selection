...
from search_space.core.space import Space
from search_space.nas_201_api import NASBench201API
from search_space.nas_201_api.models import get_cell_based_tiny_net, get_search_spaces
from search_space.nas_201_api.models.cell_searchs.genotypes import Structure


class Nasbench201(Space):
  def __init__(self, apiloc):
    self.api = NASBench201API(apiloc, verbose=False)
    self.epochs = '12'
  ...
  def random_arch(self, size):
    while True:
      index = self[random.randint(0, len(self) - 1)]
      arch = self.get_network(index)
      # if len(arch.spec.matrix) >= size:
      return arch, index
