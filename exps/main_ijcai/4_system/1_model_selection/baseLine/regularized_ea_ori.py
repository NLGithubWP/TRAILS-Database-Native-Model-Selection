...
from query_api.query_model_gt_acc_api import Gt201
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ea_cycles", type=int, default=200,  help="The number of cycles in EA.")
    parser.add_argument("--ea_population", default=10, type=int, help="The population size in EA.")
    parser.add_argument("--ea_sample_size", default=3, type=int, help="The sample size in EA.")
    parser.add_argument(
        "--time_budget",
        type=int,
        default=1200*100,
    ...
    #
    parser.add_argument(
        "--loops_if_rand", type=int, default=100, help="The total runs for evaluation."
    )
    # log
    parser.add_argument(
        "--save_dir",
    ...
    if args.rand_seed < 0:
        ...
        gt201_ins = Gt201()
        data = {}
        for run_id in range(args.loops_if_rand):
            print("{:} : {:03d}/{:03d}".format(time_string(), run_id, args.loops_if_rand))
            args.rand_seed = random.randint(1, 100000)
            save_dir, all_archs, all_total_times = main(args, api)
            all_info[run_id] = {"all_archs": all_archs, "all_total_times": all_total_times}

            # todo: mark this is new added
            test_acc_this_run = []
            for arch_id in all_archs:
                test_acc_this_run.append(gt201_ins.query_200_epoch(arch_id=str(arch_id), dataset=args.dataset)[0])

            data[run_id] = {}
            data[run_id]["arch_id_list"] = all_archs
            data[run_id]["current_best_acc"] = test_acc_this_run
            data[run_id]["x_axis_time"] = all_total_times

        with open("result_base/result_system/simulate/train_based_201_img.json", 'w') as outfile:
