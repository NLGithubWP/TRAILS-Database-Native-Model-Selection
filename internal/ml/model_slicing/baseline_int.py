...
def pre_processing(mini_batch_data: List[Tuple]):
    ...
    print("feat_id = ", feat_id.size())
    return {'id': feat_id}


def pre_processing_zero(mini_batch_data: List[Tuple]):
    """
    mini_batch_data: [('0', '0', '123:123', '123:123', '123:123',)
    """
    res = []
    for elelst in mini_batch_data:
        inner = []
        for ele in elelst[2:]:
            inner.append(int(ele))
        res.append(inner)
    return res
...
def fetch_data(database, batch_size):
    ...
    begin_time = time.time()
    rows2 = pre_processing_zero(rows)
    time_dict["type_convert"] += time.time() - begin_time

    print("Data preprocessing ....")
    begin_time = time.time()
    batch = pre_processing(rows2)
    end_time = time.time()
